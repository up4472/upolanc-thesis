...
def standardize (x : numpy.ndarray, axis : int = None) -> Tuple[numpy.ndarray, Any, Any] :
	"""
	Doc
	"""

	avg_value = numpy.mean(x, axis = axis)
	x = numpy.subtract(x, avg_value)

	std_value = numpy.std(x, axis = axis)
	x = numpy.divide(x, std_value, out = numpy.zeros_like(x), where = std_value != 0)
